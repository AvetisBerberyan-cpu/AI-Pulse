...
export async function createDocument(file) {
    ...
    try {
        ...
        await db
            .collection(documentsCollection)
            .updateOne({ _id: docResult.insertedId }, { $set: { extractedText: text, chunks, status: "ready" } });
        ...
        fs.unlinkSync(path);
        return { id: docResult.insertedId, status: "ready" };
    }
    catch (err) {
        if (err instanceof Error) {
            await db
                .collection(documentsCollection)
                .updateOne({ _id: docResult.insertedId }, { $set: { status: "error", error: err.message } });
            fs.unlinkSync(path);
            return {
